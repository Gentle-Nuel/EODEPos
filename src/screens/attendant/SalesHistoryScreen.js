...
const METHOD_ICON  = { cash: 'cash-outline', transfer: 'swap-horizontal-outline', pos_card: 'card-outline' };
...
export default function SalesHistoryScreen({ navigation }) {
  ...
  const fetchSales = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (filter === 'today') {
        ...
        query = query.gte('created_at', startOfWeek(now));
      }
  ...
  // ── Navigate to full receipt ───────────────────────────────────────────────

  const openReceipt = useCallback((sale) => {
    const attendantName = user?.full_name ?? user?.email?.split('@')[0] ?? 'Attendant';
  ...
  const totals = useMemo(() => {
    ...
    const byMethod = {};
    sales.forEach(s => {
      if (s.payment_method_1) byMethod[s.payment_method_1] = (byMethod[s.payment_method_1] ?? 0) + (s.amount_1 ?? 0);
      if (s.payment_method_2) byMethod[s.payment_method_2] = (byMethod[s.payment_method_2] ?? 0) + (s.amount_2 ?? 0);
    });
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {/* Summary card — only when data loaded */}
          ...
          <View style={styles.summaryMethods}>
            {Object.entries(totals.byMethod).map(([method, amount]) => (
              <View key={method} style={styles.summaryMethodItem}>
                <Ionicons name={METHOD_ICON[method] ?? 'cash-outline'} size={13} color={Colors.gold} />
                <Text style={styles.summaryMethodLabel}>{METHOD_LABEL[method] ?? method}</Text>
      ...
      {loading ? (
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.centeredText}>No sales yet</Text>
  ...
  return (
    <TouchableOpacity style={styles.saleCard} onPress={onPress} activeOpacity={0.75}>
      ...
      <View style={styles.saleRight}>
        ...
        <Ionicons name="chevron-forward" size={14} color={Colors.secondaryText} style={{ marginTop: 4 }} />
      </View>
    </TouchableOpacity>
  );
}
...
function MethodChip({ methodKey }) {
  ...
  return (
    <View style={styles.chip}>
      <Ionicons name={METHOD_ICON[methodKey] ?? 'cash-outline'} size={10} color={Colors.navy} />
...
const styles = StyleSheet.create({
  ...
  summaryMethodLabel:  { fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: '500' },
  ...
  saleMeta:    { fontSize: 11, color: Colors.secondaryText, marginTop: 2, marginBottom: 5 },
